...
const ROLE_DETECT_MODEL = "google/gemini-2.5-flash-lite";

// ============================================================
// User Role Detection — تشخیص جایگاه حقوقی کاربر
// ============================================================
type UserRole =
  | "judge"       // قاضی
  | "lawyer"      // وکیل مدافع/شاکی
  | "plaintiff"   // شاکی/خواهان
  | "defendant"   // متشاکی/خوانده/متهم
  | "legal_expert"// کارشناس/حقوق‌دان
  | "citizen"     // شهروند عادی
  | "unknown";

const ROLE_LABELS_FA: Record<UserRole, string> = {
  judge: "قاضی",
  lawyer: "وکیل",
  plaintiff: "شاکی/خواهان",
  defendant: "خوانده/متهم",
  legal_expert: "کارشناس حقوقی",
  citizen: "شهروند",
  unknown: "نامشخص",
};

async function detectUserRole(question: string, apiKey: string): Promise<{ role: UserRole; confidence: number; reason: string }> {
  const sys = `تو یک طبقه‌بند نقش کاربر در یک سامانه حقوقی هستی. بر اساس لحن، زاویه دید و کلمات کلیدی سوال، تشخیص بده گوینده در چه جایگاهی است.

نقش‌های مجاز:
- judge: قاضی — می‌خواهد پرونده‌ای را قضاوت کند، حکم بنویسد، ادله را وزن‌کشی کند، یا رأی صادر کند.
- lawyer: وکیل — می‌خواهد لایحه/دفاعیه/شکایت برای موکل بنویسد یا استراتژی دفاع طراحی کند.
- plaintiff: شاکی/خواهان — خودش را زیان‌دیده معرفی می‌کند و به دنبال احقاق حق است.
- defendant: خوانده/متهم — خودش را در معرض ادعا/اتهام می‌بیند.
- legal_expert: کارشناس/دانشجو/پژوهشگر حقوق — سوال تحلیلی/آموزشی می‌پرسد.
- citizen: شهروند عادی — سوال ساده و عمومی بدون ادعای طرف بودن.
- unknown: اگر مشخص نیست.

فقط JSON: {"role":"...","confidence":0..1,"reason":"..."}`;
  try {
    const r = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      method: "POST",
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        model: ROLE_DETECT_MODEL,
        messages: [
    ...
    if (!r.ok) return { role: "unknown", confidence: 0, reason: "" };
    ...
    const role = validRoles.includes(parsed.role) ? parsed.role : "unknown";
    return {
      role,
    ...
    console.warn("detectUserRole error", e);
    return { role: "unknown", confidence: 0, reason: "" };
  }
}

// دستور کاری متناسب با نقش کاربر — لحن و خروجی
function roleSpecificGuidance(role: UserRole): string {
  switch (role) {
    case "judge":
      return `\n\n[نقش تشخیص‌داده‌شده: قاضی]\nخروجی باید به سبک «پیش‌نویس رأی قضایی» باشد: بی‌طرفانه، مستدل، مبتنی بر ادله و مواد قانونی. در بخش draft به‌جای لایحه، «پیش‌نویس رأی/قرار» بنویس شامل: مقدمه، گردش‌کار پرونده، دلایل طرفین، تحلیل قضایی، مستند قانونی، و منطوق رأی.`;
    case "lawyer":
      return `\n\n[نقش تشخیص‌داده‌شده: وکیل]\nخروجی باید تاکتیکی و به نفع موکل باشد ولی درعین‌حال دفاع‌پذیر و مبتنی بر قانون. در analysis نقاط قوت/ضعف پرونده را برای وکیل بازگو کن و استراتژی دفاع/طرح دعوا پیشنهاد بده. draft = لایحه/دادخواست/شکواییه رسمی.`;
    case "plaintiff":
      return `\n\n[نقش تشخیص‌داده‌شده: شاکی/خواهان]\nبا زبان قابل فهم برای غیرحقوقی توضیح بده حقوق او چیست، بار اثبات با کیست، و چه ادله‌ای لازم دارد. draft = پیش‌نویس شکواییه/دادخواست از منظر خواهان.`;
    case "defendant":
      return `\n\n[نقش تشخیص‌داده‌شده: خوانده/متهم]\nبا زبان قابل فهم، دفاعیات و ایرادات ممکن (شکلی/ماهوی)، مرور زمان، عدم توجه دعوا و... را بررسی کن. draft = پیش‌نویس لایحه دفاعیه.`;
    case "legal_expert":
      return `\n\n[نقش تشخیص‌داده‌شده: کارشناس حقوقی]\nپاسخ می‌تواند فنی‌تر و دکترینی باشد؛ اختلاف‌نظرها، رویه قضایی و تحلیل مقایسه‌ای مفید است. draft ضروری نیست مگر کاربر بخواهد.`;
    case "citizen":
      return `\n\n[نقش تشخیص‌داده‌شده: شهروند]\nپاسخ ساده، بدون واژگان تخصصی سنگین، همراه با توضیح اصطلاحات. راهنمایی عملی برای مراجعه به دفتر خدمات قضایی/وکیل بده. draft در صورت لزوم، ساده و قابل امضا.`;
    default:
      return `\n\n[نقش کاربر تشخیص داده نشد]\nپاسخ متوازن، عمومی و بی‌طرفانه بده و در ابتدای summary به این نکته اشاره کن که برای پاسخ دقیق‌تر بهتر است کاربر جایگاه خود (شاکی، خوانده، وکیل، قاضی و...) را روشن کند.`;
  }
}

// ============================================================
// Judicial Reasoning Framework — الگوی استدلال قضایی (همیشه اعمال می‌شود)
// این چارچوب، سطح تحلیل را از "مشاوره یک‌طرفه" به "قضاوت متوازن" ارتقا می‌دهد.
// ============================================================
const JUDICIAL_FRAMEWORK = `

=== چارچوب تحلیل قضایی (Judicial Analysis Framework) ===
فارغ از نقش کاربر، تحلیل تو باید ساختار ذهنی یک قاضی حرفه‌ای را دنبال کند:

۱. **تنقیح مناط دعوا (Issue Framing):** موضوع اصلی اختلاف را دقیق و بدون ابهام مشخص کن. عناوین حقوقی احتمالی (مثلاً: مطالبه اجرت‌المثل، خلع ید، فسخ نکاح، کلاهبرداری، ...) را نام ببر.

۲. **احراز صلاحیت و شرایط شکلی:** پیش از ورود به ماهیت، بررسی کن که آیا مرجع رسیدگی صالح است، ذی‌نفعی خواهان محرز است، مرور زمان نگذشته، و شرایط اقامه دعوا/طرح شکایت رعایت شده است.

۳. **ادله و بار اثبات (Evidence & Burden of Proof):** برای هر ادعا مشخص کن: بار اثبات با کیست؟ چه ادله‌ای در پرونده هست (اسناد، شهادت، اقرار، امارات، سوگند، کارشناسی)؟ ارزش اثباتی هر یک را وزن‌کشی کن.

۴. **بررسی ادعاها و دفاعیات هر دو طرف (Adversarial Weighing):** حتی اگر کاربر فقط یک طرف را مطرح کرده، به‌صورت ذهنی قوی‌ترین دفاع/ایراد طرف مقابل را فرض کن و به آن پاسخ بده. این نکته کلیدی است — یک قاضی هرگز یک‌طرفه فکر نمی‌کند.

۵. **تطبیق قاعده بر واقعه (Subsumption):** ماده قانونی → عناصر آن → تطبیق تک‌تک عناصر بر فاکت‌های پرونده → نتیجه. تعارض قوانین، نسخ، تخصیص، حکومت و ورود را در نظر بگیر.

۶. **رویه قضایی و دکترین (Precedent & Doctrine):** اگر رأی وحدت رویه، نظریه مشورتی اداره حقوقی قوه قضاییه، یا نظر مشهور فقهی/حقوقی در این موضوع وجود دارد و از منابع استنادی قابل احراز است، به آن اشاره کن. از استناد مجعول اکیداً پرهیز کن.

۷. **نتیجه‌گیری متوازن:** حکم/نظر نهایی را همراه با درجه قطعیت («محتمل»، «قوی»، «قطعی») بیان کن. اگر نتیجه به شرایط اضافی وابسته است، سناریوها را تفکیک کن.

۸. **ملاحظات عدالت و انصاف:** جایی که قانون سکوت دارد یا اجرای خشک آن به بی‌عدالتی می‌انجامد، به اصول کلی حقوقی (لاضرر، عدالت، حسن نیت، منع سوءاستفاده از حق) اشاره کن.

این چارچوب باید در بخش analysis پاسخ منعکس شود، اما نه به‌صورت شماره‌گذاری خام؛ بلکه در قالب چند پاراگراف منسجم که مسیر استدلال قضایی از آن پیداست.
=== پایان چارچوب قضایی ===`;
